...
@plugins.register(
    ...
    namecn="lcard",
    desc="发送小胰宝定制卡片式链接和小程序",
    version="0.5",
    author="xiaoyibao-sam",
)
class lcard(Plugin):
    def __init__(self):
        super().__init__()
    ...
    def on_handle_context(self, e_context: EventContext):
        ...
        # 处理点歌命令
            ...
            keyword_raw = content[2:].strip() # 获取原始关键词
            # 对关键词进行URL编码，避免特殊字符问题，同时移除空格
            keyword_encoded = urllib.parse.quote_plus(keyword_raw.replace(" ", "")) 
            logger.info(f"[lcard] Processing song request for: {keyword_raw} (encoded: {keyword_encoded})")
            # 添加type=text参数
            url = f"https://www.hhlqilongzhu.cn/api/dg_QQmusicflac.php?msg={keyword_encoded}&n=1&type=json"
            try:
                logger.debug(f"[lcard] Requesting song info from: {url}")
                resp1 = requests.get(url)
                logger.debug(f"[lcard] API response status code: {resp1.status_code}")
                logger.debug(f"[lcard] API response headers: {resp1.headers}")
                logger.debug(f"[lcard] API response text (first 500 chars): {resp1.text[:500]}") # 记录部分原始响应文本

                # 确保请求成功并且响应类型是JSON
                if resp1.status_code == 200 and 'application/json' in resp1.headers.get('Content-Type', ''):
                    response_data = resp1.json()
                    logger.debug(f"[lcard] API response JSON: {response_data}")
                    ...
                    if response_data.get('code') == 200 and 'data' in response_data:
                        ...
                        song_url = data.get('music_url', '')
                        picture = data.get('cover', '')
                        logger.info(f"[lcard] Found song: {song} by {singer}")
                        
                        # 检查必要字段是否存在
                        if song and singer and song_url:
                ...
                logger.error(f"[lcard] JSONDecodeError processing song request for '{keyword_raw}': {je}. Response text: {resp1.text[:500]}")
                _set_reply_text(f"点歌服务返回数据格式错误，请稍后再试。", e_context, level=ReplyType.TEXT)
                e_context.action = EventAction.BREAK_PASS
                e_context['handled'] = True
                return
            except Exception as e:
                logger.error(f"[lcard] Error processing song request for '{keyword_raw}': {e}")
        ...
        # 处理临床搜索
        elif content.startswith("临床") or content.startswith("搜索临床"):
            # 提取关键词，移除前缀"用药"或"药品"并清理空格
            if content.startswith("临床"):
                keyword = content[2:].strip()
            else:  # 开头是"搜索临床"
                keyword = content[4:].strip()
                
            if not keyword:
                _set_reply_text("请按示范格式输入：临床+KRAS", e_context, level=ReplyType.TEXT)
                return
                
            logger.info(f"[lcard] 执行临床搜索: {keyword}")
            
            try:
                # 使用 app_card.py 中的 clinical_search 函数生成卡片
                xml_app = fun.clinical_search(to_user_id, keyword)
                reply = Reply(ReplyType.XML, xml_app)
                e_context['reply'] = reply
                e_context.action = EventAction.BREAK_PASS
                e_context['handled'] = True
                logger.info(f"[lcard] 成功发送临床搜索卡片: {keyword}")
                return
            except Exception as e:
                logger.error(f"[lcard] 处理临床搜索请求出错: {e}")
                _set_reply_text(f"临床搜索请求出错: {e}", e_context, level=ReplyType.TEXT)
                return
